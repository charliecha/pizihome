...
前面长篇大论的基本都在介绍概念，下面我们看看Dagger2的基本应用。关于Dagger2的依赖配置就不在这里占用篇幅去描述了，大家可以到它的github主页下去查看官方教程[https://github.com/google/dagger](https://link.zhihu.com/?target=https%3A//github.com/google/dagger)。接下来我们还是拿前面的Car和Engine来举例。
...
![img](https://pic2.zhimg.com/80/v2-576330a5bfb2b595596594a3aa36d9c9_1440w.jpg)
...
Compared to service locator: (摘自Google)

- The collection of dependencies required by a service locator makes code harder to test because all the tests have to interact with the same global service locator. （不方便测试）
- Dependencies are encoded in the class implementation, not in the API surface. As a result, it's harder to know what a class needs from the outside. As a result, changes the dependencies available in the service locator might result in runtime or test failures by causing references to fail.（违反依赖倒置原则）
- Managing lifetimes of objects is more difficult if you want to scope to anything other than the lifetime of the entire app. （对象生命周期不易管理）

## 对比Coco

Coco的问题：

1、编译慢，原因在于bpi和代码一起打包，编译时需要解压aar插件bpi文件做下沉；

2、Service Locator的问题，coco也存在。



## **一种改进的方案**

Dagger2除了可以在日常开发中用来管理对象的创建，还可以用来实现模块化。

![image-20200605103444589](upload\image-20200605103444589.png)

上图是网上找到的一个比较好的模块化解决方案，除了Utils以外，所有模块都拆分为API和Impl两部分。

API管理着模块对外提供的能力；

Impl提供API的实现和对于外部的依赖；



遵守依赖倒置原则：

API不能依赖Impl，API不能依赖API

Impl可以依赖API，Impl不可以依赖Impl
...
什么时候该采用Service Locator？

我觉得Service Locator适合单例，独立的服务

Dagger除了能实现Service Locator的能力以外，还管理相互依赖的模块之间的依赖。
